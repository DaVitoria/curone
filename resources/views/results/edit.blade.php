    ...
    <div class="content-wrapper full-page-wrapper d-flex align-items-center auth login-bg">
      <div class="card col-lg-6 mx-auto">
        <div class="card-body px-5 py-5">
            @include('messages')
          <h3 class="card-title text-left mb-3">Editar Resultado</h3>
          <form class="form-sample" method="post" action="{{route('result.update', $result->id)}}">
            @csrf
            <div class="row g-3">
              <div class="col">
                    <label>Resultados</label>
                    <select class="form-control p_input @error('result') é inválido @enderror value="{{old('result')}}" name="status" value="{{$result->status}}">
                        @error('result')
